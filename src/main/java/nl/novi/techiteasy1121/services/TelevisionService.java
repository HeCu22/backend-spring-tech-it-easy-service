...
import nl.novi.techiteasy1121.Dtos.SalesDto;
...
public class TelevisionService {
    ...
    public List<SalesDto> getAllTelevisionsSales() {
        List<Television> tvList = televisionRepository.findAll();
        List<SalesDto> salesDtoList = new ArrayList<>();

        for(Television tv : tvList) {
            SalesDto dtoSales = transferToDtoSales(tv);
            salesDtoList.add(dtoSales);
        }
        return salesDtoList;
    }
    ...
    public SalesDto transferToDtoSales(Television television){
        SalesDto dtoSales = new SalesDto();

        dtoSales.setId(television.getId());

        dtoSales.setPrice(television.getPrice());

        dtoSales.setOriginalStock(television.getOriginalStock());
        dtoSales.setSold(television.getSold());
